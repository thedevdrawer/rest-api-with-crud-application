...
function insert($fname = NULL, $lname = NULL, $phone = NULL){
    ...
    header('Location: /update?id='.$mysqli->insert_id);
    exit();
}
    ...
    if($result->num_rows !== 0):
        ...
        while($row = $result->fetch_assoc()){
            ...
            if(isset($_SESSION['user'])) {
                ...
                $_SESSION['message'] = array('type'=>'success', 'msg'=>'You have successfully created a new user, once approved you can login here');
                header('Location: /login');
            }
            exit();
        }
